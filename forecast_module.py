...
# Suprimir warnings específicos de statsmodels relacionados con frecuencia de fechas
...
def prepare_forecast_data(df, column='valor'):
    ...
    # Crear índice de fechas con frecuencia explícita
    df_clean.set_index('fecha', inplace=True)
    
    # Establecer frecuencia diaria explícitamente para evitar warnings de statsmodels
    df_clean.index = pd.DatetimeIndex(df_clean.index, freq='D')
    ...
    # Asegurar que la serie tenga frecuencia diaria
...
def exponential_smoothing_forecast(series, periods=365):
    ...
    try:
        ...
        n = len(series)
        train_size = int(0.8 * n)
        series_train = series[:train_size].copy()
        series_test = series[train_size:].copy()
        
        # Asegurar frecuencia en series de entrenamiento
        if series_train.index.freq is None:
            series_train.index.freq = 'D'
        ...
        # Asegurar frecuencia en serie completa
        if series.index.freq is None:
            series.index.freq = 'D'
...
def arima_forecast(series, periods=365):
    ...
    try:
        ...
        # Para intervalos de confianza, usar el método get_prediction con pasos específicos
        try:
            forecast_result = final_fitted_model.get_prediction(
                start=len(series), 
            ...
            # Si falla, calcular intervalos de confianza manualmente
            residuals = final_fitted_model.resid
            std_residuals = residuals.std()
            confidence_interval = 1.96 * std_residuals
            forecast_ci = pd.DataFrame({
                'lower': forecast.values - confidence_interval,
                'upper': forecast.values + confidence_interval
            })
...
def run_forecast_module():
    ...
    # Métodos avanzados con scikit-learn (comentado para ocultar)
    ...
    # Desactivar métodos de ML (comentados arriba)
    use_sklearn_linear = False
    use_sklearn_poly = False
    use_sklearn_seasonal = False
    ...
    if st.button("Generar Predicciones", type="primary"):
        ...
        if forecasts:
            ...
            # Información de los métodos (comentado para ocultar)
            # st.subheader("📋 Información de los Métodos")
            # for method, info in forecast_info.items():
            #     st.write(f"**{method}:** {info}")
              # Métricas de precisión detalladas (comentado para ocultar)
            # if forecast_metrics:
            ...
            # Estadísticas de las predicciones (comentado para ocultar)
